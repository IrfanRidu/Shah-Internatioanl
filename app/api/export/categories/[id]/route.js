...
export async function PUT(request, { params }) {
  ...
  const item = await ExportCategory.findByIdAndUpdate(params.id, body, { new: true, runValidators: true });
...
export async function DELETE(request, { params }) {
  ...
  if (!['superAdmin', 'admin'].includes(session?.user?.role)) return NextResponse.json({ success: false, message: 'Unauthorized' }, { status: 403 });
  await connectDB();
  const item = await ExportCategory.findByIdAndDelete(params.id);
  if (!item) return NextResponse.json({ success: false, message: 'Not found' }, { status: 404 });
