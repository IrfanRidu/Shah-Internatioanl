...
// Mirrors getPriceForBuyer's local/international split, for Minimum Order Quantity. Local and
// international buyers can be configured with different MOQs per product (e.g. a small local retail
// order vs. a bulk export order) via minimumOrderQuantityLocal / minimumOrderQuantityInternational on
// the product form. Falls back to the legacy single minimumOrderQuantity field (then to 1) so a
// product saved before this split existed — or one where the admin only ever filled in the old
// field — still behaves sensibly for both buyer types instead of showing "undefined units".
export const getMoqForBuyer = (product, buyerType) => {
  if (!product) return 1;
  const legacy = product.minimumOrderQuantity || 1;
  if (buyerType === 'local') {
    return product.minimumOrderQuantityLocal ?? legacy;
  }
  if (buyerType === 'international') {
    return product.minimumOrderQuantityInternational ?? legacy;
  }
  return legacy;
};
...
// Normalizes user-typed search text before it's matched against product names/descriptions: trims
// leading/trailing whitespace, strips punctuation (typing a stray comma, period, parenthesis, etc.
// shouldn't prevent a match against a clean product name), and collapses repeated internal
// whitespace to a single space. Unicode-aware (\p{L}/\p{N}) so this doesn't mangle non-Latin product
// names (e.g. Bengali local names) — only punctuation/symbols are stripped, letters and digits in
// any script are kept. Case-insensitivity itself is handled separately via the 'i' regex flag at
// each call site; this only handles whitespace/punctuation. Every "search products" surface in the
// app (storefront listing, header autocomplete, admin product list, the shipment-details product
// picker, and the admin campaign/section product pickers) ultimately goes through either this
// function (buildProductQuery, below) or /api/products/search directly — both apply this the same
// way so results are consistent everywhere.
export const normalizeSearchTerm = (input) => {
  if (!input) return '';
  return String(input)
    .trim()
...
// Builds a regex SOURCE string (not a RegExp — callers decide flags, e.g. Mongo's $options:'i' vs a
// plain JS RegExp) from already-normalized search text, tolerant of punctuation/whitespace
// differences BETWEEN words too — not just at the ends. Product names/scientific names in this
// catalog are routinely written like "Mango (Mangifera indica)"; a search for "mango mangifera
// indica" (no parens, as most people naturally type) needs to still match that. Each word is
// escaped individually and joined with \W+ (one or more non-word characters) instead of escaping
// the whole phrase as one literal block with fixed single spaces — so any run of punctuation/
// whitespace in the stored text between two searched words is accepted, while the words themselves
// still have to appear, in order, fully spelled out (this is not fuzzy/typo matching).
export const buildFlexibleSearchRegexSource = (normalizedTerm) => {
  const words = normalizedTerm.split(' ').filter(Boolean).map(escapeRegex);
  return words.join('\\W+');
};
...
export const buildProductQuery = (filters = {}) => {
  ...
  if (filters.search) {
    const cleanSearch = normalizeSearchTerm(filters.search);
    if (cleanSearch) {
      const safeSearch = buildFlexibleSearchRegexSource(cleanSearch);
      query.$or = [
