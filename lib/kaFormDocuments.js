...
// longer how expNo is entered (see expNoWithDate's comment above, issue 2 R29): the admin now
...
function assembleKaFormData(application, lang) {
  ...
  // Issue 2 (R29): EXP No. is now entered WITH the year already included by the admin (e.g.
  // "000367/2026", matching the reference document's own format) — the old code below appended
  // ANOTHER year on top of that unconditionally, producing a duplicated "000367/2026/2026". Fixed
  // by showing expNo exactly as entered (nothing appended), and properly using the shipment's own
  // expDate — a real date, not just a bare year — for the "& Date" half of this column's header,
  // which is what "EXP No. & Date" actually promises and the old code only ever half-delivered
  // (a year alone, never a full date). expDate has had its own input in the shipment editor since
  // batch 11 (R25); falls back to the shipment's own (always-populated) date field only for
  // shipments saved before expDate existed, so this is never simply blank for old data.
  const expNoWithDate = (s) => {
    const dateStr = s.expDate ? DDot(s.expDate) : (s.date ? DDot(s.date) : '');
    return dateStr ? `${s.expNo || ''}\n${dateStr}` : (s.expNo || '');
  };
  ...
  const sectionERows = shipments.map((s, i) => {
    ...
    return [
      ...
      M(invoiceValueFC), DDot(s.date), expNoWithDate(s),
