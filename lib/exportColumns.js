...
// batch 20: 'hsCode' deliberately NOT listed for bdInvoice here (unlike packingList/buyerInvoice,
// where it's still an optional toggle) — see getDocumentColumns() below for why.
...
export const DEFAULT_DOCUMENT_COLUMNS = {
  ...
  // Total CTN, Quantity KG, Unit Price, Total). batch 20: this entry is now redundant-but-harmless —
  // getDocumentColumns() force-includes 'hsCode' for bdInvoice unconditionally (see below) — kept
  // here anyway so this array still reads as the true, complete intended default column order.
...
// break rendering.
//
// batch 20: HS Code is force-included for bdInvoice specifically, regardless of what the category's
// own saved documentColumns.bdInvoice says. Previously hsCode was a normal togglable column (like
// every other one here) — but that meant ANY category whose saved config didn't happen to include it
// (unchecked in the picker, or saved by an older version of this form before hsCode existed as a
// column at all) silently dropped HS Code from every BD Invoice under that category, on every
// surface (PDF, DOCX/XLSX, the print page, and the admin editor's own BD Invoice tab) — since all of
// them derive their column list from this one function. A BD Invoice is a customs export document;
// it should never be possible to end up without an HS Code column on it, intentionally or by an old
// saved config, so it's no longer offered as an optional toggle for this document at all (removed
// from AVAILABLE_COLUMNS.bdInvoice above) and is guaranteed here instead. packingList/buyerInvoice
// are unaffected — hsCode stays a normal optional toggle for those two.
export function getDocumentColumns(category, docKey) {
  ...
  if (Array.isArray(fromCategory) && fromCategory.length > 0) {
    ...
    result = filtered.length > 0 ? filtered : (DEFAULT_DOCUMENT_COLUMNS[docKey] || []);
  } else {
    result = DEFAULT_DOCUMENT_COLUMNS[docKey] || [];
  }
  if (docKey === 'bdInvoice' && !result.includes('hsCode')) {
    result = ['hsCode', ...result];
  }
  return result;
}

// batch 17: shouldShowBdHsCode() used to gate a separate "sub-line under the name vs nothing"
// on/off switch for BD Invoice's HS Code. Removed — HS Code became a normal togglable column in
// AVAILABLE_COLUMNS.bdInvoice/DEFAULT_DOCUMENT_COLUMNS.bdInvoice, like every other column. batch 20
// then removed the toggle too (see getDocumentColumns() above) — it's unconditional now. The
// ExportCategory.bdInvoiceShowHsCode schema field itself is left in place harmlessly for any
// already-saved documents; nothing reads it anymore.
