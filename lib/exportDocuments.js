...
function drawInfoGrid(doc, y, rows) {
  const startY = y;
  doc.setFontSize(8.5);
  // Batch 19 (post-delivery fix, screenshot-reported): per-EXTRA-line height for a cell whose text
  // wraps to more than one line (see the maxLines logic below) — derived from the current font
  // size rather than a second hardcoded guess, so it always matches whatever font size this
  // function actually uses. getFontSize() (points) * 1.15 (jsPDF's own default line-height
  // factor) * 25.4/72 (pt->mm) — plain arithmetic built only on the well-established getFontSize()
  // method, not on any less-certain jsPDF-internal line-height API.
  const lineH = doc.getFontSize() * 1.15 * (25.4 / 72);
  let cursorY = y + 4;
  for (let i = 0; i < rows.length; i += 2) {
    const leftX = MARGIN + 2, rightX = MARGIN + CONTENT_WIDTH / 2 + 4;
    const pair = [rows[i], rows[i + 1]];
    const colWidth = CONTENT_WIDTH / 2 - 6;
    // The OLD code always advanced cursorY by a flat 5mm per row-pair, regardless of how many
    // lines a cell's text actually wrapped to — jsPDF's own `maxWidth` option on .text() below
    // wraps automatically when drawn, but doesn't reserve any EXTRA vertical space for that
    // wrapping on its own. A long Exporter address wrapping to 2 lines only had 1 line's worth of
    // room before the NEXT row started, so that next row was drawn on top of the wrapped 2nd line
    // — exactly the reported overlap (Exporter/Contract No, in both letterhead and plain A4).
    // splitTextToSize computes the exact same wrapping .text(maxWidth) applies internally, so
    // checking it here first tells us how tall this row actually needs to be BEFORE advancing the
    // cursor, for both cells in the pair (the taller of the two wins, same as a CSS grid row would).
    let maxLines = 1;
    pair.forEach(([label, val]) => {
      if (!label) return;
      const labelW = doc.getTextWidth(`${label}: `);
      const wrapped = doc.splitTextToSize(String(val || ''), colWidth - labelW);
    ...
    // Exactly 5mm when a row doesn't wrap (maxLines===1, the overwhelming majority of rows) — zero
    // visual change from before for those; only a row that genuinely needs more room gets it.
    cursorY += 5 + (maxLines - 1) * lineH;
  }
...
export async function generateInvoicePDF({ shipment, buyer, letterheadUrl, type, exporterInfo = DEFAULT_EXPORTER, plain = false }) {
  ...
  // Batch 19 (R33-1): Product HS Code mode — BD Invoice mirrors shipment.items directly (one row
  // per individual product, same source Buyer's Invoice already uses) instead of the category-
  // grouped bdItems. Column SET stays 'bdInvoice' either way (not 'buyerInvoice', which omits
  // totalCTN) — only the item source and the botanical-name/header treatment change with the mode.
  ...
  autoTable(doc, {
    startY: y,
    // batch 17 (R3): BD Invoice's category-mode rows are product-category names, not individual
    // products — no botanical name applies there (only Buyer's Invoice, and batch 19's Product HS
    // Code mode, show one). HS Code used to be concatenated into this same cell for BD Invoice;
    // it's a normal column now (via `columns` above), rendered generically by cellText() below.
    head: [['SL', (isBuyer || isBdProductMode) ? 'Name of Products (Botanical Name)' : 'Name of Products', ...columns.map((k) => columnHeaderLabel(k, currency, shipment.salesTerm))]],
    body: items.map((item, i) => {
      const name = (isBuyer || isBdProductMode) && item.botanicalName ? `${item.productName} (${item.botanicalName})` : item.productName;
...
function assembleDocData(docType, shipment, buyer, exporterInfo) {
  ...
  // Batch 19 (R33-1): Product HS Code mode — same reasoning as generateInvoicePDF above. docKey
  // stays 'bdInvoice' regardless of HS-code mode (the admin's configured column set for BD Invoice
  // still applies); only the item source and botanical-name treatment change.
  const isBdProductMode = baseType === 'bd-invoice' && shipment.bdHsCodeMode === 'product';
  const currency = shipment.baseCurrency || 'EUR';
  const items = ((isPacking || isBuyer || isBdProductMode) ? shipment.items : (shipment.bdItems || [])).filter((i) => i.productName);
  ...
  const showBotanicalName = isPacking || isBuyer || isBdProductMode;
...
export async function generateAllDocumentsPDF({ shipment, buyer, letterheadUrl, docStyle = 'letterhead', exporterInfo = DEFAULT_EXPORTER }) {
  ...
  // Batch 19 (R33-1): BD Invoice's availability now depends on its HS Code mode — Product mode
  // mirrors shipment.items directly (same as Buyer's Invoice, see generateInvoicePDF above) and
  // never populates bdItems at all, so checking bdItems alone would incorrectly report "nothing to
  // show" for a Product-mode shipment that actually has plenty of items.
  const bdInvoiceHasContent = shipment.bdHsCodeMode === 'product'
    ? (shipment.items || []).some((i) => i.productName)
    : (shipment.bdItems || []).some((i) => i.productName);
  const availableDocTypes = [
    ...
    { key: 'bd-invoice', has: bdInvoiceHasContent },
  ].filter((d) => d.has);
