...
// starts content at the normal top margin with no header graphic at all. See lib/pdfLetterhead.js for
...
function drawHeader(doc, y, letterhead, exporterInfo, plain) {
  if (plain) return y;
  const contentStartY = drawLetterheadBackground(doc, letterhead, PAGE_WIDTH, PAGE_HEIGHT);
  return contentStartY != null ? contentStartY : y;
}
...
const PLAIN_TABLE_STYLE = {
  theme: 'grid',
  styles: { cellPadding: 1.5, fillColor: false },
  headStyles: { fillColor: false, textColor: [0, 0, 0], fontStyle: 'bold', lineColor: [0, 0, 0], lineWidth: 0.2, fontSize: 8 },
  footStyles: { fillColor: false, textColor: [0, 0, 0], fontStyle: 'bold', lineColor: [0, 0, 0], lineWidth: 0.2, fontSize: 8 },
  bodyStyles: { fillColor: false, textColor: [0, 0, 0], lineColor: [0, 0, 0], lineWidth: 0.15, fontSize: 8 },
...
function assembleDocData(docType, shipment, buyer, exporterInfo) {
  ...
  const showBdHsCode = !isPacking && !isBuyer && shouldShowBdHsCode(shipment.exportCategory);
  const grand = grandTotals(items);
  ...
  const itemHead = ['SL', 'Name of Products (Botanical Name)', ...columns.map((k) => columnHeaderLabel(k, currency))];
  const itemRows = items.map((item, i) => {
    let name = item.botanicalName ? `${item.productName} (${item.botanicalName})` : item.productName;
    if (showBdHsCode && item.hsCode) name += ` — H.S Code: ${item.hsCode}`;
    return [i + 1, name, ...columns.map((k) => cellText(k, item, currency))];
  });
